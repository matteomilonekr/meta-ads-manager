...
async def update_ad(
    ...
    blocked = safety_guard.check_write_allowed("status", ad_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"

    await client.post(ad_id, data=payload)
    safety_guard.record_write("status", ad_id)
...
async def delete_ad(ad_id: str, ctx: Context = None) -> str:
    ...
    """
    client = get_client(ctx)
    blocked = safety_guard.check_write_allowed("delete", ad_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"
    await client.delete(ad_id)
    safety_guard.record_write("delete", ad_id)
