...
async def create_campaign(
    ...
    # Safety check — prevent account restrictions
    ...
    _ALLOWED_AD_CATEGORIES = {
        "NONE", "EMPLOYMENT", "HOUSING", "CREDIT", "ISSUES_ELECTIONS_POLITICS",
    }
    categories = []
    for cat in special_ad_categories.split(","):
        cat = cat.strip().upper()
        if cat and cat != "NONE":
            if cat not in _ALLOWED_AD_CATEGORIES:
                return f"Error: invalid special_ad_category `{cat}`. Allowed: {', '.join(sorted(_ALLOWED_AD_CATEGORIES - {'NONE'}))}"
...
async def update_campaign(
    ...
    # Safety check
    kind = "budget" if (daily_budget or lifetime_budget) else "status"
    blocked = safety_guard.check_write_allowed(kind, campaign_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"

    await client.post(campaign_id, data=payload)
    safety_guard.record_write(kind, campaign_id)
...
async def pause_campaign(campaign_id: str, ctx: Context = None) -> str:
    ...
    """
    client = get_client(ctx)
    blocked = safety_guard.check_write_allowed("status", campaign_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"
    await client.post(campaign_id, data={"status": "PAUSED"})
    safety_guard.record_write("status", campaign_id)
...
async def delete_campaign(campaign_id: str, ctx: Context = None) -> str:
    ...
    """
    client = get_client(ctx)
    blocked = safety_guard.check_write_allowed("delete", campaign_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"
    await client.delete(campaign_id)
    safety_guard.record_write("delete", campaign_id)
