    ...
    if targeting:
        ...
        if isinstance(targeting, dict):
            ...
            try:
                ...
                return f"Error: invalid JSON in targeting parameter: {e}"
    ...
    if promoted_object:
        if isinstance(promoted_object, dict):
            ...
            try:
                ...
                return f"Error: invalid JSON in promoted_object parameter: {e}"
...
async def update_ad_set(
    ...
    blocked = safety_guard.check_write_allowed(kind, ad_set_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"

    await client.post(ad_set_id, data=payload)
    safety_guard.record_write(kind, ad_set_id)
...
async def pause_ad_set(ad_set_id: str, ctx: Context = None) -> str:
    ...
    """
    client = get_client(ctx)
    blocked = safety_guard.check_write_allowed("status", ad_set_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"
    await client.post(ad_set_id, data={"status": "PAUSED"})
    safety_guard.record_write("status", ad_set_id)
...
async def delete_ad_set(ad_set_id: str, ctx: Context = None) -> str:
    ...
    """
    client = get_client(ctx)
    blocked = safety_guard.check_write_allowed("delete", ad_set_id)
    if blocked:
        return f"**Operazione bloccata (protezione anti-ban):** {blocked}"
    await client.delete(ad_set_id)
    safety_guard.record_write("delete", ad_set_id)
